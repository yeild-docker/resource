...
if [ $? -ne 0 ]; then
	...
	wget https://nginx.org/download/nginx-${nginx_ver}.tar.gz -O nginx-${nginx_ver}.tar.gz && tar -zxvf nginx-${nginx_ver}.tar.gz && cd nginx-${nginx_ver} && ./configure --prefix=$nginx_path --with-http_ssl_module --with-http_v2_module --with-http_realip_module --with-http_gunzip_module --with-http_gzip_static_module --with-stream --with-openssl=/usr/local/openssl && make && make install && cd .. && rm -rf nginx-${nginx_ver}*
	cmd_rs=$?; if [ $cmd_rs -ne 0 ]; then exit $cmd_rs; fi
