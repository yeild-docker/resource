...
./configure LDFLAGS="-L${openssl}/lib -L${sqlite3}/lib" CPPFLAGS="-I${sqlite3}/include" --enable-shared --enable-loadable-sqlite-extensions --with-system-expat --with-system-ffi --with-ensurepip=yes --with-lto --prefix=${path} ${with_openssl}
cmd_rs=$?; if [ $cmd_rs -ne 0 ]; then exit $cmd_rs; fi
