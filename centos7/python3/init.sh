...
yum install -y wget gcc make zlib zlib-devel libffi libffi-devel openssl openssl-devel
wget -c https://www.python.org/ftp/python/3.7.5/Python-3.7.5.tgz -O Python-3.7.5.tgz && tar -zxvf Python-3.7.5.tgz && cd Python-3.7.5 && ./configure --with-ssl --enable-shared --enable-optimizations --prefix=${path} ${with_openssl} && make && make install && cd .. && rm -rf Python-3.7.5*
cmd_rs=$?; if [ $cmd_rs -ne 0 ]; then exit $cmd_rs; fi
