...
wget -c https://www.python.org/ftp/python/3.7.6/Python-3.7.6.tgz -O Python-3.7.6.tgz && tar -zxvf Python-3.7.6.tgz && cd Python-3.7.6 && ./configure --enable-shared --enable-optimizations --enable-loadable-sqlite-extensions --prefix=${path} ${with_openssl} && make -j8 && make install && cd .. && rm -rf Python-3.7.6*
cmd_rs=$?; if [ $cmd_rs -ne 0 ]; then exit $cmd_rs; fi
